...
class KYCPipeline:
    ...
    async def _run_illumination(
        self, frames: list[np.ndarray], colors: list[str]
    ) -> IlluminationResult:
        try:
            return await illum_challenge.analyze_from_frames(frames, colors)
        except Exception as e:
    ...
    async def _run_face_match(
        ...
        try:
            ...
            return await document_analyzer.analyze(doc_path, frames, name)
        except Exception as e:
