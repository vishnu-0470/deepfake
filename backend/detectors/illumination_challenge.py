...
class IlluminationChallenge:
    ...
    def _run_analysis(
        ...
        # Decision
        is_real    = pass_rate >= 0.5 and mean_corr >= settings.ILLUM_CORRELATION_THRESHOLD * 0.8
        confidence = min(0.95, 0.5 + abs(mean_corr))
        risk_score = 0.0 if is_real else max(50.0, (1.0 - mean_corr) * 80)

        if is_real:
            label  = DetectionLabel.REAL
            detail = f"Face reflected {passed}/{total} illumination changes (corr={mean_corr:.2f})"
        else:
            label  = DetectionLabel.FAKE
            detail = (
                f"Face did not reflect screen illumination changes "
                f"({passed}/{total} passed, mean_corr={mean_corr:.2f} < {settings.ILLUM_CORRELATION_THRESHOLD})"
            )
