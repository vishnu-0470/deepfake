...
try:
    ...
    MEDIAPIPE_OK = False
    logger.warning("[rPPG] MediaPipe solutions API unavailable — face detection degraded")
...
class RPPGDetector:
    ...
    async def analyze(self, frames: list[np.ndarray]) -> RPPGResult:
        ...
        if len(frames) < 60:
            return RPPGResult(
                ...
                detail=f"Insufficient frames ({len(frames)} < 60 required)",
                latency_ms=0,
            )
    ...
    def _run_chrom(self, frames: list[np.ndarray]) -> RPPGResult:
        ...
        snr_ok    = signal_ratio > 0.08
        is_real   = bpm_ok and snr_ok
        ...
        if is_real:
            ...
            detail = f"Heart rate detected: {bpm:.1f} bpm (dominant {dominant_freq:.2f} Hz)"
        else:
            label = DetectionLabel.FAKE
            risk  = 75.0
            if bpm is None:
                detail = "No physiological frequency peak found in video"
            elif not bpm_ok:
                detail = f"Estimated BPM {bpm:.1f} outside normal range (45–200)"
            else:
                detail = "Signal quality too low — possible synthetic video injection"
